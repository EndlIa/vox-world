...
numeric pose fields, a carrier drag, and `Camera -> View` alone (README D48). **A camera key captures the editor's view, and a run previews through the output camera.** That is the reference product's model, taken from its camera animation subsystem: its keyframes are captured from the editor camera, and its preview swaps the rendered camera for a dedicated one that no navigation can drive. Here a camera key asks the app to copy the editor's position and rotation onto `project.camera.transform` and `mirror.camera` before it reads the value, and the fields and a carrier drag do the same copy the other way — they move the editor's view to the pose they write, so what is typed is what a key then captures. Navigation itself still writes nothing: flying around never changes the shot. `fov` is never captured, because the shot's field of view is its own value. While a clip runs the viewport renders through `mirror.camera`, with the carrier and the outline hidden, which is what makes a camera track visible at all — and navigation is never given that camera, which is exactly what the removed lock did wrong (`OrbitControls.update()` ends with `object.lookAt(target)`, and it re-aimed the pose the mixer had just applied).
