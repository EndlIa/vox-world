...
numeric pose fields, a carrier drag, and `Camera -> View` alone (README D48). **Navigation keeps the authored camera current.** With no lock the viewport is what the author aims with, so every navigation move copies the editor's position and rotation onto `project.camera.transform` and `mirror.camera` — the write a camera keyframe then records, and the same one `Camera -> View` makes (plus the carrier selection). `fov` is never adopted: it is the shot's own value. Nothing is copied while a clip runs, because the clip owns the output camera's pose then (README D48).
