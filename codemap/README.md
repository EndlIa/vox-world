...
numeric pose fields, a carrier drag, and `Camera -> View` alone (README D48). **Looking and aiming are separate.** Navigation is the editor's view and never writes the document: flying around must not change the shot. Aiming is explicit and has three paths — the `Camera` group's numeric pose fields, a drag of the carrier's gizmo, and `Camera -> View`, which adopts the editor's current position and rotation as the shot (its mirror, `View -> Camera`, moves the editor to the authored pose instead). A camera keyframe records the authored pose, so the flow is aim first, then key; `fov` is never adopted, because the shot's field of view is its own value.
