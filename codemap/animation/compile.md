...
- `values.length === times.length * valueSize` for every track, and `times` is strictly increasing (pinned by `timeline.ts`), because the authored times are already strictly ascending whole milliseconds.
