...
4. `rebuild(project)` keeps the playhead: it samples `const t = this.time`, compiles `clip = buildClip(project)`, and, when a mixer exists, discards the old action and creates a new one with the same settings — `applyLoop()` (the loop mode and `clampWhenFinished` from the transport's `looping` setting), `setEffectiveWeight(1)`, `play()`, and `paused = !running`. The action is always active; deactivating it would also drop the bindings `setTime` needs. Finally it restores `setTime(t)`. With nothing bound yet the clip is retained and the action is created on the next `bind`.
...
8. `setLoop(loop)` stores the flag in `looping` and re-applies the loop mode and `clampWhenFinished` to the current action (`applyLoop`, which also runs whenever a new action is installed, so the setting survives a rebuild; the private field is the flag's storage, and `get loop()` its only reader).
...
10. `get loop()` returns the transport's own setting — the `looping` flag, never a property of the clip — read-only and legal at any time, including before the first `bind`. It is what the app reads to tell a clip that reached its end from one that wrapped: a run of a non-looping clip is over at the final frame, which is exactly where the app stops the transport and hands the view back (README D48).
11. `dispose()` stops all actions, uncaches the mixer root, and drops the clip, action, mixer, and object map. Mirror `Object3D`s, the camera, and the project are left untouched — they outlive playback.
...
- `rebuild` preserves `time`, `running`, and the loop setting — the replacement action is installed through `applyLoop()` — so editing a keyframe or the timeline duration never jumps the playhead and never changes whether the clip repeats.
...
- `loop` is the transport's own setting, not the clip's and not the document's: `setLoop` is its only writer, it reads the same `looping` flag whose effect `applyLoop` writes onto the current action, and it is readable before any `bind`, when there is no action to apply it to.
