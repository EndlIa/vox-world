...
**【暂不实现】**本契约整体挂起（用户裁定）：本阶段不实现 `model-operations.ts`。其全部函数与类型（`optimize`、`resample`、`fillHoles`、`normalize`、`centralize`、`measureVolume`、`groupByIslands`、`invertVisibility`、`setVisibilityBySlice`、`deleteHidden`、`TransformPlan`、`VolumeReport`）不得被实现或上层契约引用；解除标记必须先取得用户同意。以下条款保留为目标状态。（`measureVolume` 的 `snapshotOrScope` 参数语义仍未裁定，进入实现前必须先定。）

**职责**：提供单个 `SceneObject` 内 Optimize、Resample、Fill Holes、Normalize、Centralize、Measure Volume、Group by Islands 和可见性批处理的纯模型规则。

**接口**：
- `optimize(snapshot, neighborhood: 6 | 18 | 26) -> UniformVoxSnapshot`。
- `resample(snapshot, { factor?, targetResolution?, samplesPerAxis: 1 | 2 | 3 }) -> UniformVoxSnapshot`。
- `fillHoles(snapshot, { fallbackColor, closing: true }) -> UniformVoxSnapshot`。
- `normalize(snapshot) -> TransformPlan`、`centralize(snapshot) -> TransformPlan`。
- `measureVolume(snapshotOrScope) -> VolumeReport`。
- `groupByIslands(snapshot, { connectivity: 6 | 26; seed: number; unhideAll: true }) -> UniformVoxSnapshot`。
- `invertVisibility(snapshot) -> UniformVoxSnapshot`、`setVisibilityBySlice(snapshot, sliceY) -> UniformVoxSnapshot`、`deleteHidden(snapshot) -> UniformVoxSnapshot`。
...
**内部**：
- 所有函数只接收一个对象的局部 `UniformVoxSnapshot` 或局部范围，并返回该对象的局部快照/报告；对象身份、SceneNode 变换和场景可见性由调用方维护。
...
- Normalize：把占用包围盒最小角平移到对象局部原点 `(-0.5, -0.5, -0.5)`；对整数体素中心等价于 `delta = -min`。返回平移计划，不改变颜色/可见性，也不改变绑定 `SceneNode` 的世界变换。
- Centralize：把占用包围盒中心移到对象局部原点 `(-0.5, -0.5, -0.5)`。精确平移可能为半整数，而 `GridPosition` 只能是整数，因此每轴取最接近的整数平移（平票向负方向），并在 `residual` 报告剩余偏差；颜色/可见性不变，绑定 `SceneNode` 不变。若调用方要求精确连续坐标，应拒绝而不是产生非整数体素键。
