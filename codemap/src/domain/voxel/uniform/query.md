...
**内部**：
- 所有查询都以一个 `VoxelReadView` 为输入；该视图属于明确的 `SceneObjectId`，不得把一个对象的查询结果与另一个对象的键混用。
...
- Edit 模式下调用方必须先通过 `EditorState.activeSceneObjectId` 取得对应视图；查询模块不负责判断哪个对象可编辑。

**依赖**：types、util/packed-int。
