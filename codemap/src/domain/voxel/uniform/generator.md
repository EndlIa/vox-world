...
- `validateGeneratorSpec(spec) -> Result<void>`、`estimateVoxelCount(spec) -> number`、`generate(spec, options?) -> UniformVoxSnapshot`。
...
**内部**：
- 生成器只产生一个 `SceneObject` 的局部 `UniformVoxSnapshot`，不创建 SceneNode、不决定对象世界变换，也不修改当前场景。
- 通用坐标规则：所有尺寸为安全正整数；体素坐标从 `0` 开始，范围为 `0 .. size - 1`；颜色默认传入的 `ColorHex`，可见性一律为 `true`。空尺寸、非整数、超上限或会导致内存溢出的 spec 返回 `invalid-generator-spec`，不生成部分结果。
...
- 生成器不读取 `SceneDocument`、Selection、TransformSession、渲染状态或 UI；它只返回局部快照，是创建新 `SceneObject`、替换活动对象体素还是进入对象 XFORM 由 application 命令决定。
- 结果不得修改输入 spec；颜色统一规范化为大写 `#RRGGBB`，坐标按稳定键排序。

**依赖**：types、util/color、util/math、util/result。
