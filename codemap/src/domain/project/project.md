...
- 项目必须恰好持有一个场景快照。场景中的多个 `SceneObject` 由 `SceneNode` 组织；项目不保存活动对象、编辑模式、体素选择、XFORM 预览或渲染运行时对象。
- `snapshot()` 返回不可变纯数据，至少包含 `scene`、`animation` 和项目设置；不得泄漏 `SceneDocument`、`SceneObject` 可变条目、Three.js 对象或内部索引。
