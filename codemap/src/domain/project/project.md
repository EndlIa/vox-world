...
**内部**：组合一个可序列化的 `SceneSnapshot`、由 `domain/animation` 唯一拥有的 `AnimationDocumentV1` 和 `ProjectCameraSettings`/`ProjectRenderSettings` 项目设置；只维护项目数据，不负责编解码、存储、选择、渲染资源或历史。
...
- 新建项目必须显式生成默认 `AnimationDocumentV1`、完整的 camera/render 设置和空 Bake Mesh manifest，不能依赖加载器补字段。
