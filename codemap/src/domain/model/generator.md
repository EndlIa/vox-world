...
- 生成器不读取 `VoxelDocument`、Selection、TransformSession、渲染状态或 UI；它只返回快照，是否替换场景、追加还是进入 NEW SCENE XFORM 由 application 命令决定。
- 结果不得修改输入 spec；颜色统一规范化为大写 `#RRGGBB`，坐标按稳定键排序。

**依赖**：voxel-types、util/color、util/math、util/result。
