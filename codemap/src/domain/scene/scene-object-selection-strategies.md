...
**接口**：
- `SceneObjectSelectionStrategy` 可辨识联合：`pick`、`outliner`。第一版不定义多选或 Select All。
- `resolveSceneObjectSelection(strategy, scene, input) -> SceneObjectSelectionResolution`。
- `SceneObjectSelectionResolution`：`sceneObjectId: SceneObjectId | null`、`warnings`。

**内部**：
- `pick` 只接受 PickService 返回的稳定 `SceneObjectId`，不接收 Three.js 对象或节点实例。
