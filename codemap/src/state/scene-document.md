...
**接口**：
- `getNode`、`getSceneObject`、`sceneObjectIds`、`nodeIds`、`snapshot`、`sceneObjectSnapshot`、`voxelView`。
...
- `voxelView(sceneObjectId)` 返回绑定该对象局部网格的只读视图；视图不得泄漏内部 `Map`、数组或可变对象，也不能用于查询其他对象。
