...
- `ToolCapabilities`：`mode: "object" | "edit"`、`editButton`、`freehand`、`rect`、`box`、`needsWorkplane`、`preview`、`selection`、`instant`。
- `ToolDraft`：本次手势累积的纯数据预览、命令意图、预览 token 和取消原因。

**内部**：
- 工具只把输入解释为对应模式的 Command 或 ToolContext 预览请求，不执行命令、不直接改 `SceneDocument`、EditorState、Selection、DOM、Three.js 或 Worker。
- Object 工具只操作 `SceneObjectId`；Edit 工具只在 `activeSceneObjectId` 的局部网格中工作。工具能力声明的 mode 必须与当前 EditorState 一致，否则不得激活或起笔。
...
- 工具在 `pointerDown` 时冻结模式、`activeSceneObjectId`、workplane 策略、Bypass 策略和 `baseSceneVersion`（对称轴与 pivot【暂不实现】）；手势过程中设置变化只影响下一次手势。
...
**依赖**：tool-context、commands、scene-types、voxel/uniform/types、util/disposable。
