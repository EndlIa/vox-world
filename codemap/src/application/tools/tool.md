...
- Object 工具只操作 `VoxObjectId`；Edit 工具只在 `activeObjectId` 的局部网格中工作。工具能力声明的 mode 必须与当前 EditorState 一致，否则不得激活或起笔。
...
- 工具在 `pointerDown` 时冻结模式、`activeObjectId`、对称轴、pivot、workplane 策略、Bypass 策略和 `baseSceneVersion`；手势过程中设置变化只影响下一次手势。
...
**依赖**：tool-context、commands、scene-types、voxel-types、util/disposable。
