...
**职责**：向工具提供受控的拾取、只读查询、工作平面、预览和命令提交能力。

**接口**：
- `pick(request)`、`pickMany(request)`、`surfaceKeysInRect(rect)`：返回普通 `PickResult` 或稳定体素键，不暴露 GPU/Three.js 对象。
- `query`：`bounds`、`has`、`visibleKeys`、`byColor`、`island`、`withinBox`、`candidatePosition`。
...
**内部**：
- `pick` 只编排 PickService；工作平面命中返回离散网格点，体素命中返回稳定键、点、法线和当前可见性。工具不得自行读取渲染器实例表。
- `surfaceKeysInRect` 在矩形手势开始或结束时获取当前 render-target 可见体素键。命令携带该集合和全深度候选集合，由 Handler 根据 Bypass 策略过滤，避免 application handler 依赖渲染实现。
...
- `settings.snapshot()` 在手势开始时复制 Bypass、Add Wall 高度、Island 连通性、Clone、NEW SCENE 等设置。手势内不得读取可变 UI 单例。
...
**依赖**：pick-service、voxel-query、commands、model-commands、generator-commands、ports、state 的只读快照、util/result。
