...
- `query`：仅查询 `EditorState.activeSceneObjectId` 对应的 `VoxelReadView`；提供 `bounds`、`has`、`visibleKeys`、`byColor`、`island`、`withinBox`、`candidatePosition`。
...
- `settings.snapshot()` 在手势开始时复制 `mode`、`activeSceneObjectId`、Bypass、Add Wall 高度、Island 连通性和 Clone 等设置。手势内不得读取可变 UI 单例；Edit 工具若发现活动对象已变化必须取消手势。
