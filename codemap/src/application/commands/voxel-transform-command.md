...
- 选择命令不携带 `VoxelSnapshot`、数组索引或渲染对象；只携带局部稳定键、颜色、种子、屏幕候选和只读策略输入。
- `workingPosition` 不是命令字段；提交坐标必须由 Handler 调用 VoxelTransformSession 的 `candidatePosition` 解析。
- Add/Subtract、rootDelta flush、entry 身份、缩放冲突和去重都由 Handler/Session 规则处理；镜像/旋转冲突规则【暂不实现】（依赖 `symmetry`）。
...
**依赖**：scene-types、voxel/uniform/types、voxel/uniform/selection-strategies、util/math、util/result（`voxel/uniform/symmetry`【暂不实现】）。
