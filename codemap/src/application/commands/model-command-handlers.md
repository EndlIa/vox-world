...
- Measure Volume：只调用 `measureVolume`，返回 `VolumeReport`、无 Patch、无 History、无场景版本变化；范围为空返回零值报告。
...
- Rotate：只支持绕当前轴 90 度 CW/CCW。Bounds pivot 用模型占用包围盒旋转并保持最小角锚定；World pivot 使用已解析到对象局部坐标的 pivot 计算后四舍五入到局部整数。XFORM 活动时此 Handler 必须拒绝并要求走 `RotateSelectionCommand`。
