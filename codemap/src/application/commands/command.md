...
- Scene 命令在 Object 模式下操作对象/节点；Voxel、VoxelTransform 和 Model 命令在 Edit 模式下必须携带并匹配 `EditorState.activeSceneObjectId`。
- 命令中的体素坐标始终是目标 `SceneObject` 的局部坐标；跨对象操作必须拆成多个命令或显式 Scene 命令。
