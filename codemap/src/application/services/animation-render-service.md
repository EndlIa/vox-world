...
**内部**：
- 开始前拒绝并发任务；先校验 `filePrefix` trim 后非空且不含 `\\/:*?"<>|`，再调用 `AnimationRenderPort.validate(settings)`。实现方必须冻结当前 `AnimationDocument`，通过 `domain/animation.validateAnimation` 针对当前 `SceneSnapshot` 校验每条轨道至少一个关键帧、Node target 存在且非根、Camera/Node target 合法，并在任何输出选择前返回可展示错误。空文档由渲染器自身的启动门禁拒绝，不是 `validateAnimation` 的判据。`mp4` 必须先通过 `AnimationOutputPort.probe()`；能力不足返回可展示原因并回退/提示 PNG，不打开文件句柄。
