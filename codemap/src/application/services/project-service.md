...
**依赖**：repository-port、project、scene-document、render-settings-service、本文件定义的 `ProjectDocumentPort`/`SnapshotPort` 应用契约；具体 codec 和 snapshot-service 只由 app composition root 注入，application 不直接 import infrastructure。
...
- 项目保存必须包含 `cameraAnimation`、当前 `camera`/`render` 设置和自持久化的 Bake Mesh manifest/资产。
...
1. 捕获项目元数据、完整 `SceneSnapshot`、cameraAnimation、`render-settings-service.toProjectSettings()` 返回的 camera/render 普通数据，以及 Bake Mesh manifest 的不可变快照。
...
- `load` 先读取字节/文档，再由 `ProjectDocumentPort.decode()` 校验当前格式、SceneSnapshot、cameraAnimation 与 Bake Mesh 资产，并通过 `render-settings-service` 校验完整的 camera/render 设置；全部成功后才一次性替换 SceneDocument、EditorState、选择、动画和项目设置。
- 未知项目版本、缺失必填字段、非法关键帧、旧字段别名或非法 camera/render 值必须拒绝整个文档；不得在加载时补齐字段或过滤坏数据。
...
- ProjectService 订阅 SceneDocument 和 render-settings-service 中项目作用域的变更并增加 `projectVersion`；仅工作区偏好或纯运行时相机移动不得把项目标为 dirty。打开、新建和 Snapshot 恢复成功后重置 `projectVersion`/`savedProjectVersion` 并建立新的 History checkpoint。
...
- 若后端不支持二进制资产，只能经用户显式同意退化为仅 SceneSnapshot/cameraAnimation 的 Quick Save，并在 UI 说明 Bake Mesh 不会自动保存；不得静默降级。
...
- `saveQuick`/`restoreQuick` 委托注入的 `SnapshotPort`（由 snapshot-service 适配）；Quick Save 持久化 `SceneSnapshot` 和 cameraAnimation，不持久化 Bake Mesh 资产、运行时 Camera Control 或播放状态。
