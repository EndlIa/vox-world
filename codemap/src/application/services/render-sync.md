...
**内部**：
- `syncInitial` 从 `SceneSnapshot` 建立场景节点、对象实例缓冲和世界变换；每个 `SceneObject` 的体素只在对象局部空间上传，世界变换由绑定 `SceneNode` 承担。
...
**依赖**：renderer-port、scene-types、scene-patch、scene-document、editor-state、voxel/uniform/patch、voxel/uniform/types。
