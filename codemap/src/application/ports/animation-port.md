...
- `AnimationSessionPort extends AnimationPreviewPort`：`load(document): Result<void, AnimationError>`、`getSnapshot(): AnimationDocument`、`hasTracksForNode(nodeId): readonly string[]`、`setDuration(durationMs): Result<void, AnimationError>`、`setLoop(enabled): Result<void, AnimationError>`、`createTrack(target, channel, timeMs, value, easing): Result<string, AnimationError>`、`deleteTrack(trackId): Result<void, AnimationError>`、`addKeyframe(trackId, timeMs, value, easing): Result<string, AnimationError>`、`replaceKeyframe(trackId, keyframeId, timeMs, value, easing): Result<void, AnimationError>`、`removeKeyframe(trackId, keyframeId): Result<void, AnimationError>`、`moveKeyframe(trackId, keyframeId, timeMs): Result<void, AnimationError>`、`selectTrack(trackId | null)`、`selectKeyframe(trackId, keyframeId | null)`、`getPlaybackState()`、`play()`、`pause()`、`stop()`、`seek(timeMs)`、`tick(nowMs)`、`setFollowCamera(enabled)`、`setPathVisible(trackId, enabled)`、`stopAndClearOverride()`、`addCameraKeyframeFromControl(): Result<void, AnimationError>`、`cameraToView()`、`viewToCamera()`、`setControlTransform(field, value)`、`setControlSelected(selected)`、`subscribe(listener)`。
...
- `getPlaybackState()` 返回只读运行时状态：`{ status: 'stopped' | 'playing' | 'paused'; currentTimeMs: number; hasOverride: boolean; followCamera: boolean; selectedTrackId: string | null; selectedKeyframeId: string | null }`。它不返回 `AnimationDocument`，也不暴露时钟句柄或可变 override 容器。
...
- `getSnapshot()` 返回 `AnimationDocument` 的只读深快照；轨道和关键帧编辑返回明确 `Result`，失败不得产生部分文档变更。`createTrack()` 成功值是新分配的稳定 `trackId`；`addKeyframe()` 成功值是新分配的稳定 `keyframeId`，调用方不得猜测或用数组索引代替。`addKeyframe()` 命中已占用时间时按 `domain/animation` 的替换规则覆盖该时间点上的旧关键帧，旧 `keyframeId` 一并作废，成功值仍是本次新分配、且确实存在于文档中的 `keyframeId`。
...
- `addKeyframe()` 覆盖已占用时间上的旧关键帧时，端口实现必须发出 `warning` 级别的应用事件/通知，说明被覆盖的时间点、所属 `trackId` 与替换后的新 `keyframeId`（由 `ui/editor-view-model` 投影、`ui/controls` 显示）；不得静默覆盖，也不得在 `domain/animation` 内产生用户提示。
...
- `hasTracksForNode(nodeId): readonly string[]` 是节点删除和场景 undo/redo 的引用预检入口：返回引用该节点的全部 node 轨道 `trackId`，未被引用时返回空数组。只读、无副作用，不修改文档或播放状态；应用层据此构造 `node-referenced-by-animation`（携带 `nodeId` 与全部 `trackIds`），端口不得自行级联删除轨道。
