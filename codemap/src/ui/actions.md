...
- `animation.addKeyframe(trackId, timeMs, value, easing?)`、`animation.replaceKeyframe(trackId, keyframeId, timeMs, value, easing?)`、`animation.removeKeyframe(trackId, keyframeId)`、`animation.moveKeyframe(trackId, keyframeId, timeMs)`、`animation.selectKeyframe(trackId, keyframeId | null)`：身份只使用稳定 `trackId`/`keyframeId`，不使用列表索引或显示名称；`value` 必须匹配 target/channel。`addKeyframe` 成功结果必须返回由 `AnimationSessionPort` 经 `IdGeneratorPort` 分配的新 `keyframeId`；命中已占用时间时按 domain 规则覆盖该时间点上的旧关键帧并作废其旧 `keyframeId`，应用层必须给出 `warning` 提示。`replaceKeyframe`/`moveKeyframe` 保留原 `keyframeId`，且把关键帧放到已被其它关键帧占用的时间必须拒绝（`keyframe-time-occupied`），不得覆盖或交换时间。`removeKeyframe` 只接受该稳定 ID。`easing` 省略等价于 `"linear"`，由应用层在调用端口前补齐；文档中的关键帧必须带 `easing`。
