...
**内部**：组合项目元数据、场景快照、`AnimationDocumentV1`、camera/render 设置和 Bake Mesh manifest；只处理纯数据，不解释 Three.js 规则、不写 repository、不修改运行时状态。
...
- `AnimationDocumentV1`、`AnimationTrackV1`、`AnimationEvaluation`、`AnimationCameraPose` 等公开类型与运行时形状由 `domain/animation` 唯一拥有；project codec 只嵌入、序列化和校验该值，不重新声明平行 DTO。
...
- `animation` 必须是完整的 `AnimationDocumentV1`。`version` 必须为 `1`，`durationMs` 必须为有限正数，`loop` 必须为布尔值，`tracks` 必须是数组；未知版本、缺字段、非法字段或非法关键帧使整个文档失败，不进行关键帧过滤。
...
- Snapshot 条目必须包含完整 `SceneDocumentV1` 和 `AnimationDocumentV1`，并使用该条目自己的 `data.scene` 执行与项目文档相同的校验：文档内轨道 ID 唯一、单轨关键帧 ID 唯一、每轨至少一个关键帧、`durationMs` 为有限数且 `> 0`、同一 `target` + `channel` 唯一、节点目标存在且非根；旧 `cameraAnimation` 同样拒绝。
