...
**内部**：组合 legacy-voxel-codec、project-codec、snapshot-archive-codec、repository 和迁移链；不访问 DOM、不生成缩略图、不处理外部归档导入。
**依赖**：repository-port、project-codec、legacy-voxel-codec、snapshot-archive-codec、animation。
...
| 数据 | 键 | 说明 |
| --- | --- | --- |
| Quick Save | `vbstore_voxels` | 单槽，恢复体素和可选 cameraAnimation |
| Snapshot 记录 | `vbstore_voxels_snapshot${slot}` | `slot` 从 0 开始 |
| Snapshot 缩略图 | `vbstore_voxels_snap_imgshot${slot}` | PNG/JPEG data URL |
...
Quick Save 使用同一 `StorageRecordV2`，名称固定为 `Quick Save`，不保存缩略图。旧纯字符串和 `StorageRecordV1` 通过 `migrations` 在读取时迁移；cameraAnimation 缺失时恢复空默认动画，并清除运行时 Camera Control。

## 保存事务

1. 捕获不可变的体素字符串和 cameraAnimation 快照。
2. 校验名称：trim 后非空、长度受限；缩略图必须是允许的图片 data URL。
3. 用 repository 估算记录与缩略图的 UTF-16 总配额，但最终仍必须处理真实 `QuotaExceededError`。
4. 读取旧记录和旧缩略图到内存，作为回滚值。
5. 先提交 Snapshot 记录；成功后才提交缩略图。这里的“记录优先”只决定提交顺序，用户可见结果仍是原子槽位：缩略图失败时恢复旧记录与旧缩略图。
6. 全部写入和长度校验成功后发布 `updated` 事件；任何失败都恢复旧槽位，并返回 `QUOTA_EXCEEDED`、`SERIALIZATION_FAILED` 或 `ROLLBACK_FAILED`。
...
- `restoreSnapshot` 先完整解码、迁移和校验，再一次性替换体素与动画。失败时当前项目和槽位均不变。
- 名称 trim 后不能为空；默认名使用 `Snapshot ${slot + 1}`。同一名称允许存在，但 UI 必须以槽位 id 定位，禁止按名称选择记录。
- 空槽、超出槽位范围、未知高版本和损坏记录返回明确错误；不得把空槽恢复为空白项目。
...
- 缩略图是可选视觉附件，但一旦请求保存缩略图，就必须按原子槽位语义处理；不能留下“新体素 + 旧缩略图”或反向组合。
- 配额失败不改变 dirty 状态。Quick Save 或用户显式启用的降级 autosave 失败时保留项目 dirty，等待用户处理。

## 本地备份与恢复

- `createLocalBackup` 读取所有占用槽位，通过 `snapshot-archive-codec` 生成 Blob；没有缩略图的历史槽位仍备份体素数据，Quick Save 是否包含由显式选项决定，默认不包含。
- `restoreLocalBackup` 仅接受由本应用生成的备份 Blob/句柄或已完成校验的内部归档，不是通用外部 ZIP 导入入口。
- 恢复前先验证整个归档，保存当前占用槽位的内存回滚快照，再清空目标槽位并写入。任何解析、迁移、配额或写入失败都恢复原槽位。
