...
| 数据 | 键 | 说明 |
| --- | --- | --- |
| Quick Save | `vox-world.quick-save` | 单槽，恢复完整 `SceneSnapshot` 和 cameraAnimation |
...
Quick Save 使用同一 `StorageRecordV1`，名称固定为 `Quick Save`，不保存缩略图。记录只接受当前版本和完整字段；未知版本、缺字段或损坏 JSON 直接返回错误，不尝试迁移。

## 保存事务

1. 捕获不可变的 `SceneSnapshot` 和 cameraAnimation 快照。
...
- `restoreSnapshot` 先完整解码并校验场景与动画，再一次性替换场景与动画。失败时当前项目和槽位均不变。
