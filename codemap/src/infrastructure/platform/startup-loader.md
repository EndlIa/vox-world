...
- `user/startup.json` 的字节交给 ProjectService/project-codec 做当前格式解码和严格校验，loader 不自行理解或改写场景结构、对象局部体素字符串、`AnimationDocumentV1` 或渲染字段；未知版本、旧 `cameraAnimation` 字段和任何迁移/兼容输入都必须由 ProjectService 拒绝，不得由 loader 补字段或修复。
