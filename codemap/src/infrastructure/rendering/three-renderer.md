...
- `applyPatch` 消费 `ScenePatch`，按 `SceneNodeId`/`VoxObjectId` 更新节点层级、世界变换和对象实例资源；不得把所有对象合并成一个无身份的全局面缓冲。
...
**依赖**：three、application/ports/renderer-port、scene、voxel-instances、overlays、post-pipeline、picker、domain/scene/scene-types、domain/scene/scene-patch。
