...
**职责**：按 `SceneObject` 高效渲染大量局部体素，并维护每个对象的分块实例缓冲、局部实例矩阵、可见性、颜色和连续预览壳；只消费对象局部快照和对象级补丁，不修改领域状态。
**接口**：
- `create`、`createSceneObject`、`applySceneObjectPatch`、`setChunk`、`rebuildBounds`、`setVisibility`、`dispose`。
...
**内部**：
- 每个 `SceneObjectId` 拥有独立的实例命名空间和局部包围盒；实例矩阵只由该对象的局部 `VoxelKey` 生成，父 `SceneNode` 负责世界变换。不同对象的相同局部键不得互相覆盖。
...
**依赖**：three、voxel-material、domain/scene/scene-types、domain/voxel/uniform/types、domain/voxel/uniform/patch。
