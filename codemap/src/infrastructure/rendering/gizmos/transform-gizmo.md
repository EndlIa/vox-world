...
**内部**：
- Object 模式将手柄附着到选中 `SceneObject` 绑定的 `SceneNode` 世界代理上，输出节点局部/父空间变换意图；Edit 模式只附着活动对象的体素 XFORM 预览代理，不得附着非活动对象。
...
- 对 Object XFORM 只发出临时 SceneTransform 意图，由 SceneObjectTransformSession/Handler 决定预览、冲突和提交；对 Voxel XFORM 只发出活动对象的局部变换意图，由 VoxelTransformSession/Handler 处理；对 CameraControl 只更新运行时姿态，不自动写入关键帧。
- context lost、目标销毁或模式切换必须解绑事件和释放 helper；不保留跨上下文 GPU 句柄。

**依赖**：three、state/scene-object-selection、state/voxel/uniform/voxel-selection、state/scene-object-transform-session、state/voxel/uniform/voxel-transform-session、scene-types。
