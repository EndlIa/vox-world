...
- 场景同步：`syncScene(sceneSnapshot, settings)`、`updateObject(objectId, snapshot)`、`updateMaterials(settings)`、`updateEnvironment(settings)`、`updateLighting(settings)`、`updatePlane(settings)`、`setShadeMode(mode)`、`invalidate(reason)`。
...
- 场景从当前 `SceneSnapshot` 重建节点层级和每个 `VoxObject` 的局部 mesh buffer；节点世界变换由 SceneNode 提供。黑色体素使用 emissive/shade 材质，其他体素使用带 vertex colors 的 `MeshPhysicalMaterial`。Shade Mode 只替换渲染材质，不改变体素颜色数据。所有 mesh 必须设置 frustum culling，并按设置接收/投射阴影。
...
**依赖**：three、three/examples OrbitControls、three-gpu-pathtracer、path-tracer、camera-controller、render-settings-service、domain/scene/scene-types、domain/voxel/voxel-types、application/ports/renderer-port。
