...
- 场景同步：`syncScene(sceneSnapshot, settings)`、`updateSceneObject(sceneObjectId, snapshot)`、`updateMaterials(settings)`、`updateEnvironment(settings)`、`updateLighting(settings)`、`updatePlane(settings)`、`setShadeMode(mode)`、`invalidate(reason)`。
...
- 场景从当前 `SceneSnapshot` 重建节点层级和每个 `SceneObject` 的局部 mesh buffer；节点世界变换由 SceneNode 提供。黑色体素使用 emissive/shade 材质，其他体素使用带 vertex colors 的 `MeshPhysicalMaterial`。Shade Mode 只替换渲染材质，不改变体素颜色数据。所有 mesh 必须设置 frustum culling，并按设置接收/投射阴影。
...
- 平面默认尺寸 `0`（关闭）、颜色 `#90A0B3`；开启时位于当前场景有效对象世界包围盒最低 Y 下方 `0.5`，接收阴影但不改变对象边界。阴影地面仅在标准渲染模式可见，PathTracer 模式下隐藏。
...
**依赖**：three、three/examples OrbitControls、three-gpu-pathtracer、path-tracer、camera-controller、render-settings-service、domain/scene/scene-types、domain/voxel/uniform/types、application/ports/renderer-port。
