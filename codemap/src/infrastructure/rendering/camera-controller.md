...
- `frameAll(bounds)`、`frameColor(hex, bounds)`、`frameVoxels(bounds)`、`frameIsland(bounds)`，分别对应 `Frame All`、`Frame Color`、`Frame Voxels`、`Frame Island`；边界由应用层从当前场景、活动对象或选择集解析，控制器只负责取景。
...
- `applyAnimationOverride(patch: Partial<AnimationCameraPose>)`、`clearAnimationOverride()`；分别应用和清除相机轨道求值产生的运行时覆盖。
...
- 六个预设固定为 Right、Left、Top、Bottom、Front、Back，分别对应 `+X/-X/+Y/-Y/+Z/-Z`；切换后目标保持当前场景或选区中心，并同步 Sandbox 相机。
- `frameAll` 使用当前场景全部有效可见对象的世界包围盒；`frameColor`、`frameVoxels`、`frameIsland` 使用应用层提供的活动对象/选区边界。取景偏移默认 `1.7`，以包围球半径、垂直 FOV 和 aspect 计算安全距离，目标 Y 取包围盒高度的 `40%` 位置；取景过程不得与导航手势并发。
...
- 自动旋转默认关闭，支持 CW/CCW；速度使用 `±0.1`，空闲等待和加速时间默认各 `1` 秒。自动旋转是相机行为，不写入相机轨道关键帧。
...
- 必须实现 Auto Rotate 的 CW/CCW、速度、空闲等待和加速语义；自动旋转不得写入相机轨道关键帧。
