...
**内部**：
- SceneObject Selection 高亮绑定对象的世界包围盒/节点；Voxel Selection 高亮只属于 `activeSceneObjectId`，并携带对象身份以防对象切换后残留。
...
**依赖**：three、application/ports/animation-port、renderer-port 定义的只读选择/预览 DTO、domain/animation、domain/scene/scene-types、domain/voxel/uniform/types。不得 import application/tools/tool-context 或其他具体用例模块。
