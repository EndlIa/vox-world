...
**职责**：将逐帧 PNG Blob 通过 WebCodecs 编码为 H.264，并使用 `mp4-muxer` 写入单个 MP4 文件；负责编码器能力检查、时间戳、backpressure 和 writer 收尾，不负责统一动画求值或用户文件选择。
...
- `addFrame` 用 `createImageBitmap(blob)` 生成 `VideoFrame`；writer 根据创建时 settings 的 `fps` 调用 `getAnimationFrameTimestampUs(frame, fps)` 计算时间戳，并计算 duration `Math.round(1_000_000 / fps)` 微秒，调用方不传 `timestampUs`。仅 `frame === frameStart` 编码为 keyframe。
...
**依赖**：WebCodecs、mp4-muxer、animation/mp4-timing、application/ports/animation-port 定义的 `AnimationOutputWriter`/`AnimationOutputResult`。
