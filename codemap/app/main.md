...
6. **Flow wiring**, the only place the modules meet:
   - Import — `importFile(file)` is the body of `pickImportFile` and of the drop callback: `file.arrayBuffer()` → `await importGlb(data)`; failure →
     `panels.reportError(`${error}: ${detail}`)`; success → `lastImport = adoptImportedScene(project, scene)`, then `attachSourceMeshes(scene,
     bySourceId)` builds one `new THREE.Mesh(node.geometry, node.sourceMesh.material)` per imported node, keeps each in the app-local `sourceMeshes`
     array for teardown, and hands it to `mirror.attachSourceObject(objectId, mesh, node.matrixWorld)` — with the node's own baked world matrix (D25), not
     with an assumed identity local transform, because the object's transform is about to be replaced by the payload's placement. Then mark dirty, `bindingsDirty`,
     `mirror.sync()`, `commitDirty()`, and **immediately** `await runVoxelizeJob(buildVoxelizeSources(scene), importTarget(), bySourceId)` (D26): the import
     itself voxelizes the whole imported scene, so no click separates the import from voxels. `importTarget()` is `{ kind: 'uniform', voxelSize:
     defaults().uniformVoxelSize }`, the uniform default the panel seeds itself with. The intermediate `sync()`/`commitDirty()` gives the frames that run
     during chunked voxelization placeholder nodes and the raw meshes on layer 2; the job's own `frameAll` then measures the finished voxels. An outline
     shell is adopted and displayed like every other node but never appears in `buildVoxelizeSources`, so no payload is attached to it and its object
     stays the `'empty'` placeholder — exactly the state an un-voxelized object has, and the reason the shells are still listed and still drawn as raw
     meshes (README D27).
   - Voxelize — one body, `runVoxelizeJob(sources, target, bySourceId)`, shared by the import path and by `applyRevoxelize`: abort the in-flight job
     (`jobController?.abort()`), own a fresh `AbortController`, then `voxelize({ sources, target, budget: DEFAULT_CELL_BUDGET, onProgress, signal })`;
     failure → `reportError` with the literal and detail; success → `applyVoxelizeResult(project, result, { attachTo: bySourceId })`, mark dirty, `commitDirty()`,
     then `mirror.frameAll(viewportCamera)`; `clearProgress()` on both paths. Framing belongs here, after the payloads: an object that was an `'empty'`
     placeholder until this call renders as voxels now, and its raw mesh hides behind them unless the override is on. `frameAll` syncs before measuring, so the
     payload meshes rebuilt for the ids just marked dirty are exactly what it fits. The abort is mandatory and comes first: a setting committed twice in a
     ...
     re-voxelize) and otherwise re-sources the retained import whole with `buildVoxelizeSources(lastImport.scene)`; there is no per-object scope any more.
...
- Importing is never a state the user has to finish: a successful import always ends in a voxelization of the whole imported scene at `importTarget()`
  (README D26), so every imported object holds a payload and no click, settings change, or confirmation is needed to see voxels. There is no per-object or
  per-scope voxelize path left, and `applyRevoxelize` is the only other caller of the shared body. "Every imported object" means every object the import
  produced a source for: an outline shell is adopted and displayed but never sourced, so it stays `'empty'` — a deliberate exception, not a job that did
  not finish.
...
- Every imported node gets exactly one raw mesh on layer 2, attached to that node's object by `attachSourceMeshes` right after
  `adoptImportedScene` and handed that node's own baked `node.matrixWorld` (D25); the meshes share the imported geometry and materials, the app keeps
  them in `sourceMeshes`, the mirror never disposes them, and teardown detaches them. The matrix is what keeps the raw mesh on its imported pose once the
  object's transform has been replaced by the payload's translation: the app never re-parents a mesh, never computes a placement, and never writes a mesh
  matrix itself — that is the mirror's `applySources`. Whether they are shown is the mirror's flag and the panel's checkbox (`setSourceVisible` +
  `sceneVisible`) — `main` keeps no copy of it and never toggles `visible` on a mesh itself.
...
panel has already done its part by forwarding a target. An aborted job's own `'cancelled'` result is reported like any other failure, because
`jobController` is the only thing that knows the job was superseded. The
...
- `../voxels/voxelize/voxelize.js` — `voxelize`, `VoxelizeTarget`; `../three-runtime/import.js` — `importGlb`, `adoptImportedScene`,
  `buildVoxelizeSources`.
...
Importing is now the first assertion of every walk (README D26): dropping `island.glb` must show voxels with no further interaction — `imported N
node(s)` immediately followed by `voxelized N cell(s)` in the status line, every sourced object listed as `uniform` with a resolution, every outline shell
listed as `empty` (it is displayed, not voxelized — README D27), and no `Voxelize` button or `Whole scene` checkbox anywhere in the panel — and committing
...
panel's field values must come from the outline-free bounds: importing `stylized_forest_-_scene_may_holiday.glb` (79 nodes, 22 of them `*_Line _0` shells
holding 6410 of its 17628 triangles) reports `voxelized 32854 cell(s) from 11218 triangle(s)` — exactly the 57 non-outline nodes — lists 22 `empty` rows
beside 58 voxelized ones, and seeds voxel size `0.147 m` and root size `14.1552 m`, i.e. `root / 96` of that model's own extent. The repo's demo GLBs have
...
non-uniform scales must land with every object translation-only and every voxel grid axis-aligned in world space, and `Show raw meshes` must bring the raw
...
Scenario A exercises the camera lock end to end: import `island.glb`, detach the car, tick `Camera lock (output)`, steer the output camera
