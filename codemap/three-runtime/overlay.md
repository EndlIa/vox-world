...
class Overlay {
  ...
  showBox(boxLocal: IntBox3, matrixWorld: THREE.Matrix4, color?: HexColor): void;
  clear(): void;
  dispose(): void;
}
```
...
3. `showBox(boxLocal, matrixWorld, color?)`: normalize the inclusive `IntBox3` with `normalizeBox(boxLocal.min, boxLocal.max)`, take the min-corner convention's local extents as `min` to `max + 1` — a cell is the world unit, so no spacing is applied (README D41) — and set the wireframe matrix to `matrixWorld × translate(center) × scale(size)`. The frame therefore shows exactly the voxel space the edit will write, in the owning object's space and orientation.
...
- Conventions hold: `showBox` takes an inclusive integer box in the owning object's cells plus that object's world matrix, so the drawn frame is the region an edit writes.
...
## Errors
- `RangeError` when a `boxLocal` corner is not a finite integer.
