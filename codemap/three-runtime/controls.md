...
- `onOrbitChange`, `setOrbitTarget`, and `setViewFrom` throw nothing and are legal at any time; retargeting to the camera already in use costs one `update()` and changes nothing else, and `setViewFrom` takes the pose it is given verbatim, deriving the pivot from it and the radius itself — or taking the `target` verbatim when one is handed in.
