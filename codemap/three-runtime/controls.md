...
class ViewportControls {
  ...
  setViewFrom(position: THREE.Vector3, quaternion: THREE.Quaternion): void;   // points the viewport at a pose
  update(): void;
  dispose(): void;
}
```
...
10. `setViewFrom(position, quaternion)` points the viewport at a pose, which is what the app's `View -> Camera` means (README D46): it copies the pose onto `orbit.object` — the camera navigation drives — takes the radius navigation already orbits from (`max(orbit.object.position.distanceTo(orbit.target), MIN_ORBIT_RADIUS)`, so a viewport sitting on its own pivot still gets a usable one), puts `orbit.target` at the point the pose looks along, `position + forward · radius` (forward being `(0, 0, -1)` rotated by the quaternion), calls `orbit.object.lookAt(orbit.target)`, and then `orbit.update()`. The `lookAt` is what makes the result an orbit pose rather than an arbitrary one — a bank the pose carries is not representable, so it is dropped — and `update()` rebuilds the offset and zoom state orbits and dollies work from, so the first orbit after the jump behaves like any other. It is the only path in this file that sets a camera pose directly rather than through user input, and it writes no project state, no document, and no flag.
...
- `setViewFrom` keeps the viewport an orbit camera rather than an arbitrary one: it moves the camera onto the pose and re-derives the pivot from it, so a bank the pose carries is dropped — `lookAt` cannot express it — and the first orbit after the jump starts where the jump left off, at the radius the navigation already used. It writes nothing the app owns: no document, no project, no flag (README D46).
...
- `onOrbitChange`, `setOrbitTarget`, and `setViewFrom` throw nothing and are legal at any time; retargeting to the camera already in use costs one `update()` and changes nothing else, and `setViewFrom` takes the pose it is given verbatim, deriving the pivot and the radius itself.
