...
region operations (fill, clear, paint, extract) and allocation-free integer keys. It is not an octree, holds no
transform, identity, or scene state, and allocates no Three.js object per cell.
...
3. Min-corner convention: a voxel of size `voxelSize` at cell `(x, y, z)` occupies `[x*voxelSize, (x+1)*voxelSize]` on each axis. The grid stores integers only and never converts to meters or world space; that conversion is the owning object's transform.
...
- `UniformGrid.create` throws `RangeError` when `voxelSize` is not a finite positive number.
